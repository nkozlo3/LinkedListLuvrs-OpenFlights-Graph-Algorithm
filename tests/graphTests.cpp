...
TEST_CASE("Graph sourceToDestLongLat outputs correct data", "[weight=5][graph][7]")
{
    ...
    REQUIRE(m[str[0]].at(0).sourceAirportCode_vertex1 == "AMD");
    REQUIRE(m[str[0]].at(1).sourceAirportCode_vertex1 == "BLR");
    REQUIRE(m[str[0]].at(2).sourceAirportCode_vertex1 == "BOM");
    REQUIRE(m[str[0]].at(3).sourceAirportCode_vertex1 == "CCJ");
    REQUIRE(m[str[0]].at(4).sourceAirportCode_vertex1 == "COK");
    REQUIRE(m[str[0]].at(5).sourceAirportCode_vertex1 == "DEL");
    REQUIRE(m[str[0]].at(6).sourceAirportCode_vertex1 == "DMM");
    REQUIRE(m[str[0]].at(7).sourceAirportCode_vertex1 == "HYD");
    REQUIRE(m[str[0]].at(8).sourceAirportCode_vertex1 == "KWI");
    REQUIRE(m[str[0]].at(9).sourceAirportCode_vertex1 == "MAA");
    REQUIRE(m[str[0]].at(10).sourceAirportCode_vertex1 == "TRV");
    REQUIRE(m[str[0]].at(11).sourceAirportCode_vertex1 == "ATH");
    REQUIRE(m[str[0]].at(12).sourceAirportCode_vertex1 == "CMB");
    REQUIRE(m[str[0]].at(13).sourceAirportCode_vertex1 == "DUS");

    for (size_t i = 0; i < str.size(); i++)
    {
        std::string dest = m[str[i]].at(0).sourceAirportCode_vertex1;

        for (size_t j = 0; j < m[str[i]].size(); j++)
        {
            std::string dest = m[str[i]].at(j).sourceAirportCode_vertex1;
            REQUIRE(m[str[i]][j].lonAndLatPoints.first == m1[dest].first);
            REQUIRE(m[str[i]][j].lonAndLatPoints.second == m1[dest].second);
            REQUIRE(m[str[i]][j].distance_weight == graph.sourceToDestLongLatHelper(m1[str[i]].first, m1[str[i]].second, m1[dest].first, m1[dest].second));
        }

        REQUIRE(m[str[i]].at(0).lonAndLatPoints.first == m1[dest].first);
        REQUIRE(m[str[i]].at(0).lonAndLatPoints.second == m1[dest].second);
        REQUIRE(m[str[i]].at(0).distance_weight == graph.sourceToDestLongLatHelper(m1[str[i]].first, m1[str[i]].second, m1[dest].first, m1[dest].second));
    }
}
